...
$kode_produk = isset($_GET['kode']) ? trim($_GET['kode']) : null;
...
// Ambil data produk
try {
    $stmt = $pdo->prepare("SELECT p.*, k.nama_kategori FROM produk p LEFT JOIN kategori k ON p.id_kategori = k.id_kategori WHERE p.kode_produk = :kode_produk");
...
<head>
  ...
  <title>Edit Produk - DrinkCashier</title>
...
<body>
  ...
  <main class="main">
    <div class="topbar">
      <h1>Edit Produk</h1>
      <p>Edit data produk <?= htmlspecialchars($produk['nama_produk']) ?></p>
    </div>
      ...
      <div class="form-box">
        ...
        <div class="form-grid">
          <div class="input-group">
            ...
            <input type="text" name="kode_produk" value="<?= htmlspecialchars($produk['kode_produk']) ?>" placeholder="Masukkan kode produk" required readonly>
          </div>

          <div class="input-group">
            <label>Nama Produk</label>
            <input type="text" name="nama_produk" value="<?= htmlspecialchars($produk['nama_produk']) ?>" placeholder="Masukkan nama produk" required>
          </div>
          ...
          <div class="input-group">
            ...
            <input type="number" name="harga_produk" value="<?= $produk['harga_produk'] ?>" placeholder="Masukkan harga produk" required>
          </div>

          <div class="input-group">
            <label>Jumlah Stok</label>
            <input type="number" name="jumlah_stok" value="<?= $produk['jumlah_stok'] ?>" placeholder="Masukkan stok produk" required>
          </div>
          ...
          <div class="input-group">
            ...
            <select name="status_produk" required>
              ...
              <option value="Tersedia" <?= $produk['status_produk'] == 'Tersedia' ? 'selected' : ''; ?>>Tersedia</option>
              <option value="Habis" <?= $produk['status_produk'] == 'Habis' ? 'selected' : ''; ?>>Habis</option>
            </select>
          </div>
        </div>
        ...
        <div class="input-group" style="margin-top:25px;">
          ...
          <textarea name="deskripsi_produk" placeholder="Masukkan deskripsi produk..."><?= htmlspecialchars($produk['deskripsi_produk'] ?? '') ?></textarea>
        </div>

        <!-- UPLOAD GAMBAR DENGAN INPUT FILE TERSEMBUNYI -->
        <div class="upload-box" onclick="document.getElementById('file-input').click();">
          <i class="ri-image-add-line"></i>
          <h3>Ubah Gambar Produk</h3>
          <p>Klik di sini untuk mengganti foto produk (JPG, PNG, WEBP). Biarkan kosong untuk tetap menggunakan gambar lama.</p>
        ...
        <div class="preview-card">
          <div class="preview-image">
            <?php if ($produk['gambar_produk']): ?>
              <img src="uploads/<?= htmlspecialchars($produk['gambar_produk']) ?>" alt="<?= htmlspecialchars($produk['nama_produk']) ?>" style="width: 100%; height: 100%; object-fit: cover;">
            <?php else: ?>
              <i class="ri-image-line"></i>
            <?php endif; ?>
          </div>
          <h3><?= htmlspecialchars($produk['nama_produk']) ?></h3>
          <p><?= htmlspecialchars($produk['deskripsi_produk'] ?? 'Tidak ada deskripsi') ?></p>
          <div class="preview-price">Rp <?= number_format($produk['harga_produk'], 0, ',', '.') ?></div>
        </div>
      </div>
  ...
  <script>
    ...
    const deskripsInput = document.querySelector('textarea[name="deskripsi_produk"]');
    ...
    deskripsInput.addEventListener('input', function() {
